...
/**
 * 通过发送带有特殊ssid的probe request包，把真实的配网信息传递给模块。 
 * 项目名称：EasyLink_minus 
 * 创建人：Rocke
...
public class ProbeReqData {
	...
	/**
	 * 0x01 flag <Data…> 
	 * 第一个字节始终是0x01，表示是EasyLink Minus配网数据。 
	 * Flag：一个字节定义如下：
	 ...
	 * 
	 * Version版本定义 
	 * 1. Version=1. 原始数据由ssid_len, <ssid>, <key> 3部分组成： Ssid_len Ssid Key 只由一个probe request组成，
	 * 由于ssid最长32字节，去掉头部的0x01和flag，这里的Data数据最多只能有30个字节，即ssid和key经过原始数据转换为Data之后的总长度不能大于30字节.
	 * 
	 * 2. Version=2. 原始数据由ssid_len, key_len, <ssid>, <key>, <extra data>组成如果产生的Data长度大于30字节，则需要在增加Version=3的数据发送后续数据。
	 * 3. Version=3. 是对version=2的扩充。
	 * 其他version暂时不使用。
	 * 
	 * @param ssid
	 * @param key
	...
	/**
	 * Data: 由于加密数据是一组Hex数据，需要对加密数据依次做如下变化，产生Data数据。 
	 * 1. 7个字节依次分组，最后一组可能不足7个字节。
	 ...
	 * 产生的Data就是 {0x7E, 0x01, 0x19, 0x7E, 0x02, 0x12, 0x13, 0x34, 0x45, 0x03, 0x56, 0x78 ,0x7E, 0x02}
	 * 
	 * @param data_in
	 * @return
	 */
